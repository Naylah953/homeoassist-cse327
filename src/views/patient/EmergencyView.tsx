      ...
      <div className="flex flex-col items-center py-10 px-6 rounded-2xl text-center"
        ...
        {!called ? (
          <>
            ...
            <h2 className="text-[25px] font-bold mb-2" style={{ fontFamily: 'var(--font-display)', color: '#e0ebe2' }}>Emergency SOS</h2>
            <p className="text-[15px] mb-6 max-w-sm" style={{ color: 'rgba(224,235,226,0.6)', lineHeight: 1.6 }}>
            ...
            <button onClick={() => setCalled(true)}
              className="px-10 py-3.5 rounded-2xl text-[15px] font-bold transition-all hover:scale-105 active:scale-95 cursor-pointer"
          ...
          <>
            ...
            <p className="text-[14px] mb-4" style={{ color: '#7a7468' }}>AI triage assistant is collecting your details. Estimated wait: &lt; 2 minutes.</p>
            <button onClick={() => setCalled(false)}
              className="text-[15px] font-medium transition-colors hover:opacity-70 cursor-pointer"
      ...
      <div className="grid grid-cols-2 gap-5">
        ...
        <Card>
          <div className="px-5 py-4" style={{ borderBottom: '1px solid #d6d0c8' }}>
            <h2 className="text-[20px] font-bold" style={{ fontFamily: 'var(--font-display)' }}>On-Call Doctors</h2>
            <p className="text-[15px] mt-0.5" style={{ color: '#7a7468' }}>Real-time availability</p>
          </div>
          <div className="divide-y" style={{ borderColor: '#ede9e3' }}>
            {ON_CALL_DOCTORS.map((doc, i) => (
              <div key={i} className="px-5 py-3.5 flex items-center gap-3">
                <div className="w-8 h-8 rounded-full flex items-center justify-center text-[15px] font-bold flex-shrink-0"
                  style={{ background: '#d8f3dc', color: 'var(--color-primary)' }}>
                  {doc.name.split(' ').slice(1).map(n => n[0]).join('')}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-[16px] font-medium" style={{ color: '#1b2d20' }}>{doc.name}</p>
                  <p className="text-[13px] font-medium" style={{ color: '#7a7468' }}>{doc.specialty} Homeopathy</p>
                </div>
                <div className="text-right">
                  <Badge label={doc.status === 'available' ? 'Available' : 'Busy'} variant={doc.status === 'available' ? 'success' : 'warning'} />
                  <p className="text-[14px] mt-1" style={{ color: '#7a7468', fontFamily: 'var(--font-mono)' }}>{doc.wait}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <Card className="p-5 flex flex-col gap-4">
          <div>
            <p className="text-[15px] font-bold uppercase tracking-widest mb-3" style={{ color: '#a20000' }}>Emergency Helpline</p>
            <a href="tel:18001234567"
              className="flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-[16px] transition-colors hover:opacity-90"
              style={{ background: '#d8f3dc', color: 'var(--color-primary)', fontFamily: 'var(--font-mono)', textDecoration: 'none' }}>
              <Ico d={IC.phone} size={20} />
              1800-123-4567
            </a>
            <p className="text-[14px] mt-2" style={{ color: '#7a7468' }}>Toll-free · Available 24×7 · All Bangladesh</p>
          </div>

          <div style={{ borderTop: '1px solid #ede9e3', paddingTop: 16 }}>
            <p className="text-[15px] font-bold uppercase tracking-widest mb-2" style={{ color: '#7a7468' }}>How It Works</p>
            {['Call or tap the SOS button above', 'AI voice assistant collects your symptoms', 'Priority matched to next available doctor', 'Doctor connects within minutes'].map((step, i) => (
              <div key={i} className="flex items-start gap-3 mb-2">
                <span className="w-5 h-5 rounded-full flex items-center justify-center text-[14px] font-bold flex-shrink-0 mt-0.5"
                  style={{ background: '#131f16', color: '#e0ebe2' }}>{i + 1}</span>
                <p className="text-[14px]" style={{ color: '#1b2d20' }}>{step}</p>
              </div>
            ))}
          </div>
        </Card>
