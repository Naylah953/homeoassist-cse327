...
export function AIChatView({ onProfileClick }: Props) {
  ...
  const loadCDSS = useCallback(async (sessionId: number) => {
    setLoadingRecs(true)
    try {
      const res = await cdssApi.recommendFromSession(sessionId)
      setRecs(res.data)
    } catch (err) { console.error(err) }
        ...
        <div className="flex-1 overflow-y-auto">
          {sessions.length === 0 ? (
            ...
            <button key={s.id} onClick={() => selectSession(s)}
              ...
              <p className="text-[12px] font-medium truncate" style={{ color: '#1b2d20' }}>{s.patient_name || `Patient #${s.id}`}</p>
          ...
          <>
            <div className="px-6 py-3 flex items-center gap-3" style={{ borderBottom: '1px solid #d6d0c8', background: '#d8f3dc55' }}>
              <div className="w-8 h-8 rounded-full flex items-center justify-center text-[11px] font-bold flex-shrink-0"
                ...
                {(selected.patient_name || 'P').split(' ').map(n => n[0]).join('').slice(0,2)}
              </div>
              <div>
                <p className="text-[13px] font-medium" style={{ color: '#1b2d20' }}>{selected.patient_name || `Patient #${selected.id}`}</p>
              </div>
            ...
            {selected.status === 'completed' && recs.length === 0 && (
          ...
          {recs.length > 0 && recs.map((rec, i) => (
            <Card key={i} className="p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-[12px] font-semibold" style={{ fontStyle: 'italic', color: '#1b2d20' }}>{rec.name}</p>
                <Badge label={rec.potency} />
              </div>
              <div className="flex items-center gap-2 mb-2">
                <div className="flex-1 h-1.5 rounded-full overflow-hidden" style={{ background: '#ede9e3' }}>
                  <div className="h-full rounded-full" style={{ width: `${rec.score}%`, background: i === 0 ? 'var(--color-accent)' : 'var(--color-primary)' }} />
                </div>
                <span className="text-[10px] font-bold" style={{ fontFamily: 'var(--font-mono)', color: i === 0 ? 'var(--color-accent)' : 'var(--color-primary)' }}>{rec.score}%</span>
              </div>
              <div className="flex flex-wrap gap-1 mb-1">
                {rec.indications.map((ind, j) => (
                  <span key={j} className="text-[10px] px-2 py-0.5 rounded-full" style={{ background: '#ede9e3', color: '#7a7468' }}>{ind}</span>
                ))}
              </div>
              <p className="text-[10px]" style={{ color: '#7a7468', fontStyle: 'italic' }}>{rec.dosage}</p>
            </Card>
          ))}
